...
class SendRTPServiceTest {
  ...
  @Mock private SepaRequestToPayMapper sepaRequestToPayMapper;
  @Mock private ReadApi readApi;
  ...
  @BeforeEach
  void setUp() {
    sendRTPService = new SendRTPServiceImpl(sepaRequestToPayMapper, readApi, serviceProviderConfig);
    inputRtp =
        Rtp.builder()
  ...
  void testSend() {
    ...
    var expectedRtp =
        Rtp.builder()
            ...
            .resourceID(ResourceID.createNew())
            .savingDateTime(LocalDateTime.now())
            .rtpSpId(activationRtpSpId)
            .endToEndId(endToEndId)
            .iban(iban)
            .payTrxRef(payTrxRef)
            .flgConf(flgConf)
            .build();
    SepaRequestToPayRequestResourceDto mockSepaRequestToPayRequestResource =
        new SepaRequestToPayRequestResourceDto(URI.create("http://callback.url"));
    ...
    StepVerifier.create(result)
        .expectNextMatches(
            rtp ->
                rtp.noticeNumber().equals(expectedRtp.noticeNumber())
  ...
  void givenPayerIdNotActivatedWhenSendThenMonoError() {
    ...
    StepVerifier.create(result).expectError(PayerNotActivatedException.class).verify();
