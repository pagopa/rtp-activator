...
import it.gov.pagopa.rtp.activator.statemachine.TransitionConfiguration;
...
class StateMachineConfigurationTest {
  ...
  void givenStateMachineConfig_whenCreatingConfigurer_thenAllTransitionsAreRegistered() {
    ...
    assertAll(
        ...
        () -> assertTransitionExists(config, RtpStatus.ACCEPTED, RtpEvent.USER_ACCEPT_ACCEPTED_RTP, RtpStatus.USER_ACCEPTED),
        () -> assertTransitionExists(config, RtpStatus.ACCEPTED, RtpEvent.USER_REJECT_ACCEPTED_RTP, RtpStatus.USER_REJECTED),
        () -> assertTransitionExists(config, RtpStatus.ACCEPTED, RtpEvent.CANCEL_ACCEPTED_RTP, RtpStatus.CANCELLED),
        () -> assertTransitionExists(config, RtpStatus.USER_ACCEPTED, RtpEvent.PAY_USER_ACCEPTED_RTP, RtpStatus.PAYED),
        () -> assertTransitionExists(config, RtpStatus.USER_ACCEPTED, RtpEvent.CANCEL_USER_ACCEPTED_RTP, RtpStatus.CANCELLED),
